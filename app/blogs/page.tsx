...
export default function BlogsPage() {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      ...
      const data: BlogApiResponse = await res.json(); // ✅ typed
      setBlogs(data.blogs || []);
    };
    fetchBlogs();
  }, []);
  ...
  return (
    <div className="min-h-screen font-sans">
      ...
      <div className="banner-bg mx-6 mt-4 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center p-6 md:p-8 text-white">
        <div className="max-w-full md:max-w-[50%] mb-4 md:mb-0">
          ...
          <div className="search-wrapper relative w-full max-w-sm">
            <input
              ...
              className="search-input w-full pl-9 pr-10 py-2 rounded-full border-2 border-orange-500 text-sm outline-none bg-transparent text-white transition-all duration-300 ease-in-out placeholder:text-gray-400 cursor-pointer hover:border-orange-500 hover:shadow-lg focus:border-orange-500 focus:shadow-lg"
            />
            <Search 
        ...
        <div className="w-full md:w-[40%] h-[120px] md:h-[150px] bg-blog-banner bg-no-repeat bg-cover bg-center rounded-xl brightness-90"/>
      </div>
        ...
        {filteredBlogs.length ? (
          filteredBlogs.map((blog) => (
            <div
              ...
              role="button"
              tabIndex={0}
              onKeyDown={(e) => { if (e.key === "Enter") handleCardClick(blog.slug); }}
            >
              <Image
                src={blog.imgsrc}
                alt={blog.title}
