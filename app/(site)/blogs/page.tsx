    ...
    <div className="bg-[#fafaf9] min-h-screen">
      ...
      <section className="relative overflow-hidden py-24 md:py-32">
        ...
        <div className="relative max-w-3xl mx-auto px-5 md:px-8 text-center">
          ...
          <motion.p {...fade(0.14)} className="text-base md:text-lg text-zinc-500 max-w-xl mx-auto mb-8">
          ...
          <motion.div {...fade(0.2)} className="relative max-w-md mx-auto">
            ...
            <input
              ...
              className="w-full rounded-xl border border-zinc-200 bg-white pl-9 pr-4 py-2.5 text-sm text-zinc-700 placeholder-zinc-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-400/40 focus:border-orange-300 transition"
            />
          </motion.div>
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-5 md:px-8 pb-24 md:pb-32">
        {loading ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="rounded-2xl border border-zinc-200/70 bg-white p-5 space-y-3 animate-pulse">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((blog, i) => (
              <motion.article
                key={blog.id}
                {...fade(i * 0.05)}
                className="group relative flex flex-col rounded-2xl border border-zinc-200/70 bg-white hover:border-orange-200 hover:shadow-md transition-all duration-300 overflow-hidden"
              >
                <div className="flex-1 p-5 space-y-3">
                  ...
                  <h2 className="text-sm font-semibold text-zinc-900 leading-snug group-hover:text-orange-500 transition-colors">
                    {blog.title}
                  </h2>
